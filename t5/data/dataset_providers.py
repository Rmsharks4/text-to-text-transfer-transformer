...
class PlaceholderPreprocessor(object):
  """Base class for placeholders in preprocessing pipeline."""

  def __call__(self, dataset):
    raise RuntimeError(f"{self.__name__} should never be called.")


class CacheDatasetPlaceholder(PlaceholderPreprocessor):
  """A placeholder to signal when in the pipeline offline caching will occur."""
  pass


class TokenizeDatasetPlaceholder(PlaceholderPreprocessor):
  """A placeholder to signal when in the pipeline tokenization will occur."""
  pass
...
class TaskV3(DatasetProviderBase):
  ...
  def __init__(
    ...
    Args:
      ...
      dataset_fn: a function with the signature
      ...
      output_features: dict(str, Feature), output features of the Task to be
        ...
        ensure they include featuers that match this specification. Note that
      ...
      preprocessors: list(callable), an optional list of functions that receive
        ...
        `output_features`.
      postprocess_fn: callable, an optional function that receives decoded
        model outputs and converts them to a form that is ready for evaluation
        using the metric functions in `metric_fns`.
      metric_fns: list(callable), an optional list of metric functions with the
        ...
        If undefined or empty, no evaluation will occur on the task.
      ...
      for prep in preprocessors[:cache_step_idx]:
        if "sequence_length" in inspect.signature(prep).parameters.keys():
          raise ValueError(
              ...
              f"before `CacheDatasetPlaceholder` in {name}.")
  ...
  @property
  def metric_fns(self)->Optional[Iterable[Callable[..., Mapping[str, float]]]]:
  ...
  def _preprocess_dataset(
    ...
    """Sequentially applies preprocessors including tokenization placeholder."""
    for prep_fn in preprocessors:
      if isinstance(prep_fn, TokenizeDatasetPlaceholder):
        # Tokenize
        dataset = utils.encode_string_features(
            dataset, self.output_features, keys=self.output_features,
            copy_plaintext=True)
      else:
  ...
  def get_dataset(
    ...
    if not copy_plaintext:
      ds = ds.map(
          lambda x: {k: v for k, v in x.items() if not k.endswith("_plaintext")}
      )
...
class Task(TaskV3):
  ...
  def __init__(self,
    ...
    preprocessors.append(TokenizeDatasetPlaceholder())
    if supports_caching:
